...
| B | 引擎运行器与服务桥接 | engine/ + server/ | 进行中：B-1、B-2、B-3 完成（注册表与目录；框图 JSON 装载器与数据解析器；观察者、平台层、观测点产品），P1-4a 频谱分析完成；引擎 50 项单测全绿；B-4 起未开始 | B-1 注册表与参数描述 → B-2 JSON 装载 → B-3 观察者与产品 → B-4 `cuav_run` → B-5 任务管理 → B-6 WS 补取 → B-7 视窗抽取；关浏览器任务不停；断线按 `since` 补取无缺号 | 示例框图装载运行：`total_samples` 补齐 2000000、谱 1953 行、包络 489 行；13 项装载器用例覆盖 17 个错误码中的 16 个 | 06 §9A；WORKLOG 2026-09-05「B-2」 |
