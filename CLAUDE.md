...
| `docs/` | 代码引用的规范：iq-format / api-versions / scene-package / display-route / **diagram-format / scenario-format / display-products / component-catalog**（2026-09-04 新增，字段已冻结，D-030）/ emitter-template（2026-09-05 草案，D-045）/ schemas/ / decisions | 否 |
...
- 前端与服务（已可用）：`cd server && npm run build && node dist/index.js` 起服务（8080），`cd web && npm run build` 出产物，浏览器开 `http://127.0.0.1:8080/`；`npm test` 在 server（102 项，含真引擎集成，二进制缺失则跳过）与 web（87 项，`node:test` + `tsx`，含前端归约对 `tests/golden/product-window.json` 的 12 用例对拍）各自可跑；端到端 `node tests/e2e/scene-smoke.mjs`（14 项）与 `node tests/e2e/slice1-smoke.mjs`（39 项，U-1 + U-3：切换视图、提交任务、断线补取、dBm 轴标、信号视图跟随 / 回看、缩放重取、marker 与游标键、结束收口、CSV、长任务，需真引擎；脚本在 stderr 打进度）。前端开发者模式 `?dev=1`（显示溯源 / 标定来源 / 分色，挂 `window.__cuav.{ws.dropForTest(), signal.zoomTo() / reset() / csv(), perf.reset()}`，探针 `app.perf.longTasks`，`performance.measure('signal.render' | 'stream.flush')`）；`web/src/api/ws.ts` 客户端约定见 `docs/api-versions.md` §4.0。
...
- 视窗抽取端点（已可用，B-7，`docs/display-products.md` §3、`docs/api-versions.md` §3.1b）：`GET /api/v1/results/{task}/{op}/spectrum?t0&t1&f0&f1&px&py&stat`、`.../envelope?t0&t1&px`（Float32 小端行主序，维度与实际范围在 `X-CUAV-*` 头）、`.../{kind}/index`（索引原文 + `rows_available` + `index_final`）、`.../{track|links|detections}?t0&t1&stride`（JSON 数组，生产者待 G 线）；`scatter` 暂 404。读端以文件长度定行数，产品或索引未就绪回 409 `not_ready` + `Retry-After`，超 16 MB 回 413 带必定能过的 `suggest`。参考实现与黄金基准：`uv run --quiet python algos/reference/product_window.py <产品目录> --kind spectrum --px 800 --py 400 --stat mean --out <文件>`、`... --golden tests/golden/product-window.json`。
...
- 引擎运行器 `cuav_run`（已可用，B-4；构建产物 `engine/build/cuav_run`）：`--catalog`（目录 JSON，黄金基准 `tests/golden/component-catalog.json`）、`--validate <框图.json>`（只校验不落盘）、`--run <框图.json> --out data/runs/<task_id> [--task-id <id>] [--seed N] [--resolved <旁挂.json> | --data-index <index.manifest.json>...] [--progress-interval-ms N]`；stdout 每行一条 JSON 事件并镜像到 `<out>/events.jsonl`，退出码 0–4，约定见 `docs/api-versions.md` §4.1；`--scenario-track` 待 G-2；产品目录格式见 `docs/display-products.md`。
...
| U | 人机交互界面 | web/ | U-0 前半已写 2026-09-04（v1.6）；**U-1 三视图壳完成 2026-09-06**；**U-3 信号视图完成 2026-09-06**（频谱 + 瀑布 + 包络条，跟随 / 回看，marker、游标、导出；web 87 项单测、`slice1-smoke` 39 项；D-048）；U-2、U-4 未开始；IQ 散点待引擎 `iq` 产品（D-040 ③） | U-0 报告 09 → U-1 三视图壳 → U-2 React Flow 画布（目录驱动、连线校验与 `--validate` 同源、观测点）→ U-3 信号视图（频谱、瀑布、包络、IQ 散点，按视窗抽取）→ U-4 结果页与数据中心 | — | 06 §9B |
| G | 场景与目标活动生成 | docs + geo/ + engine/ + web/ | 未开始（切片 ②）；PROVISIONAL | G-0 场景规范 → G-1 geo 运动学与链路预算 → G-2 `ScenarioSource` → G-3 施加信道 → G-4 编辑器 → G-5 态势图层 → G-6 活动时间线；预览与 C++ 航迹差 ≤ 1e-6 度；界面不做合成标记（D-043） | — | 06 §9C；`docs/scenario-format.md` |
