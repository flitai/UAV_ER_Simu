...
| `docs/` | 代码引用的规范：iq-format / api-versions / scene-package / display-route / **diagram-format / scenario-format / display-products / component-catalog**（2026-09-04 新增，字段已冻结，D-030）/ emitter-template（2026-09-05 草案，D-045）/ **diagram-canvas-guide（2026-09-07，框图画布的操作说明与约束，面向使用者不是规范）** / schemas/ / decisions | 否 |
...
| 框图画布 | **React Flow**（`@xyflow/react` 12.11.6，2026-09-07 锁版随包，13 个传递依赖已进 `THIRD-PARTY-NOTICES`）；本项目自身状态管理仍是零依赖 `useReducer` + context；路由手写 hash | D-032 |
...
| U | 人机交互界面 | web/ | U-0 全篇已写（前半 2026-09-04，后半 2026-09-07，v1.8）；**U-1 三视图壳完成 2026-09-06**；**U-3 信号视图完成 2026-09-06**（频谱 + 瀑布 + 包络条，跟随 / 回看，marker、游标、导出；web 87 项单测、`slice1-smoke` 39 项；D-048）；**U-2 React Flow 画布完成 2026-09-07**（目录驱动、连线判据只查 `port_compat`、参数面板由 `ParamSpec` 生成、拓扑分层布局；web 131 项单测、`slice3-smoke` 27 项）；U-4 缩为任务列表与数据中心，未开始；**2026-09-07 D-051：框图页主形态改为典型链路视图（C 线 C-7 / U-5），自由画布降为高级模式 `#/diagram/canvas`**；IQ 散点待引擎 `iq` 产品（D-040 ③） | U-0 报告 09 → U-1 三视图壳 → U-2 React Flow 画布（目录驱动、连线校验与 `--validate` 同源、观测点）→ U-3 信号视图（频谱、瀑布、包络、IQ 散点，按视窗抽取）→ U-4 结果页与数据中心 | — | 06 §9B |
| C | 典型链路与检测识别评价 | web/ + engine/ + server/ + algos/ + models/ | **C-0 设计、C-1 格式与接口、C-2 天线 / 前端 / ADC 三件均完成 2026-09-07**：端口类型 7 种、`PortSpec.optional`、框图 `template_ref`、`SceneParamFrame` 七字段、`BlockMeta.clip_count`、场景 `emission.polarization`；`AntennaGain` / `ReceiverFrontEnd` / `AdcQuantizer` 落地，两个既有组件加参且缺省保旧行为，**组件目录 12 → 15，04 §8.1 的六类首次全部有组件**（D-050 销项）。引擎 142 项单测 + 8 项 ctest、服务 111 项、web 134 项、四套 e2e 全绿；**C-6 框图落盘与结果端点、C-7 典型链路视图完成**：框图页默认是九环节固定链，自由画布降为 `#/diagram/canvas`；框图落盘 `data/diagrams/`；**切片 ④a 收口，S1 对链路预算差 0.45 dB、S2 底噪对解析差 0.02 dB**。引擎 142 项单测 + 8 项 ctest、服务 116 项、web 149 项、五套 e2e 全绿；C-3 起未开始 | C-0 方案 → C-1 格式与接口 → C-2 天线 / 前端 / ADC 三件 → C-3 检测升级与落盘 → C-4 特征与识别 → C-5 真值与评价 → C-6 框图落盘与结果端点 → C-7 典型链路视图 → C-8 `demo-02` 与 G-6 → C-9 结果页两页签 → C-10 DSP 厚化接入 → C-11 收口；切片 ④ 重排为 ④a–④d | **已达成（C-2）**：拆开天线后与折在一处的接收电平差 **< 0.001 dB**（线 0.05）；S2 底噪与 −174 + nf + 10·log10 fs 差 **< 0.05 dB**（线 0.3）；ADC 量化信噪比与 6.02·bits + 1.76 dB 差 < 0.5 dB。**待 C-5**：`metrics.json` 与 Python 参考逐值一致、跨层算例 ① 典型链路实例差 ≤ 0.05、识别准确率 ≥ 0.9（原型阶段验证值） | 06 §9G；10 报告 §8 |
