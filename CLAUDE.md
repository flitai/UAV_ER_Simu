...
- 前端与服务（已可用）：`cd server && npm run build && node dist/index.js` 起服务（8080），`cd web && npm run build` 出产物，浏览器开 `http://127.0.0.1:8080/`；`npm test` 在 server（58 项，含 1 项真引擎集成，二进制缺失则跳过）与 web（6 项）各自可跑；端到端 `node tests/e2e/scene-smoke.mjs`。
