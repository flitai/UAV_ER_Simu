...
| `data/`（待建） | IQ、场景数据包、黄金基准 | 大文件不入 git，只入索引与元数据 | 每个产物挂同构元数据 |
...
| AOI、首批频段/带宽/机型 | 04 附录 B 清单 | 阶段 0 |
...
| D0 | 场景数据包 | scene/ | 未开始 | AOI basemap.pmtiles + buildings.geojson（解码合并，含 src）+ dem + manifest，全离线可加载 | — | — |
