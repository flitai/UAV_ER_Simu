...
- 前端与服务（已可用）：`cd server && npm run build && node dist/index.js` 起服务（8080），`cd web && npm run build` 出产物，浏览器开 `http://127.0.0.1:8080/`；`npm test` 在 server（102 项，含真引擎集成，二进制缺失则跳过）与 web（43 项，`node:test` + `tsx`）各自可跑；端到端 `node tests/e2e/scene-smoke.mjs`（14 项）与 `node tests/e2e/slice1-smoke.mjs`（23 项，U-1：切换视图、提交任务、断线补取、dBm 轴标，需真引擎）。前端开发者模式 `?dev=1`（显示溯源 / 标定来源 / 分色，挂 `window.__cuav.ws.dropForTest()`）；`web/src/api/ws.ts` 客户端约定见 `docs/api-versions.md` §4.0。
...
| U | 人机交互界面 | web/ | U-0 前半已写 2026-09-04（v1.6）；**U-1 三视图壳完成 2026-09-06**（路由、store、REST / WS 客户端、顶栏与徽标、运行组、抽屉与状态条、JSON 框图页、结果页信号页头、探针 `app`；web 43 项单测、`slice1-smoke` 23 项）；U-2 起未开始 | U-0 报告 09 → U-1 三视图壳 → U-2 React Flow 画布（目录驱动、连线校验与 `--validate` 同源、观测点）→ U-3 信号视图（频谱、瀑布、包络、IQ 散点，按视窗抽取）→ U-4 结果页与数据中心 | — | 06 §9B |
