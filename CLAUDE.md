...
14. **合成与估算数据的标记留在数据层**：`src=est:*`（估算建筑高度）、合成底噪等标记随字段传到每个下游产物、清单与文档（04 §9.2），任何计算与统计都能据此区分；**界面不解释数据来源与估算**（演示系统只要求数据合理可算，D-042），来源分色与占比只在开发者模式 `?dev=1` 作内部诊断。「不得用于验收指标」这类限定写在文档与模型卡里，不写在界面上。
...
| B | 引擎运行器与服务桥接 | engine/ + server/ | 进行中：B-1 至 B-5 完成（注册表与目录；框图 JSON 装载器与数据解析器；观察者、平台层、观测点产品；`cuav_run` 可执行与事件流；服务任务管理器：提交→同步校验→排队→运行→终态，取消、退出收尾与重启对账），P1-4a 频谱分析完成；引擎 57 项单测 + 3 项冒烟、服务 58 项测试全绿；B-6 起未开始 | B-1 注册表与参数描述 → B-2 JSON 装载 → B-3 观察者与产品 → B-4 `cuav_run` → B-5 任务管理 → B-6 WS 补取 → B-7 视窗抽取；关浏览器任务不停；断线按 `since` 补取无缺号 | 示例框图装载运行：`total_samples` 补齐 2000000、谱 1953 行、包络 489 行；`cuav_run --run` 同一框图 2442 行产品事件、`events.jsonl` 与 stdout 逐字节相同、同种子产品逐字节复现；目录黄金基准 8 组件；经 `POST /api/v1/tasks` 提交同一框图 1953 + 489 行、`--validate` 约 9 ms、墙钟 0.14 s；真实片段 `dronerfb_0_CH0_S4` 回放 3906 + 977 行、事件与 `task.json` 无服务器路径 | 06 §9A；WORKLOG 2026-09-05「B-2」「B-4」「B-5」 |
