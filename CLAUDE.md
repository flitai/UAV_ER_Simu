...
| B | 引擎运行器与服务桥接 | engine/ + server/ | 进行中：B-1 至 B-6 完成（注册表与目录；框图 JSON 装载器与数据解析器；观察者、平台层、观测点产品；`cuav_run` 可执行与事件流；服务任务管理器：提交→同步校验→排队→运行→终态，取消、退出收尾与重启对账；WS 订阅与补取、`product_row` 二进制帧），P1-4a 频谱分析完成；引擎 57 项单测 + 3 项冒烟、服务 72 项测试全绿；B-7 起未开始 | B-1 注册表与参数描述 → B-2 JSON 装载 → B-3 观察者与产品 → B-4 `cuav_run` → B-5 任务管理 → B-6 WS 补取 → B-7 视窗抽取；关浏览器任务不停；断线按 `since` 补取无缺号 | 示例框图装载运行：`total_samples` 补齐 2000000、谱 1953 行、包络 489 行；`cuav_run --run` 同一框图 2442 行产品事件、`events.jsonl` 与 stdout 逐字节相同、同种子产品逐字节复现；目录黄金基准 8 组件；经 `POST /api/v1/tasks` 提交同一框图 1953 + 489 行、`--validate` 约 9 ms、墙钟 0.14 s；真实片段 `dronerfb_0_CH0_S4` 回放 3906 + 977 行、事件与 `task.json` 无服务器路径；WS 回放切片 ① 2447 条事件（2442 帧二进制）82 ms，断线以 `since` 重连首条 = since + 1、并集无缺号 | 06 §9A；WORKLOG 2026-09-05「B-2」「B-4」「B-5」「B-6」 |
