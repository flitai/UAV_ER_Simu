...
| `07/08/09.*.md` | 设计报告（09 前半 v1.1 已写并经评审修订 2026-09-04，其余待写）：07 = D3-0 遮挡落点与坐标基座；08 = P1-0 引擎分块流与多速率（含注册表、JSON 装载、观察者、参数帧与 IQ 时钟对齐、Coder 产物包装）；09 = 主界面、三视图与框图画布交互设计（顶替缺失的原型 html，PROVISIONAL，D-034） | 否 |
