...
/*
 * CUAV_PFB_M16  M = 16 的换向器一拍。窗口与抽头都是 P = 29·M = 464。
 *    子信道数必须是 codegen 时的常量：reshape 的行数与 ifft 的长度都靠它。
 *    于是每个 M 一个入口，C 里也就是一个符号（生成的函数名跟着 .m 的名字走）。
 *    算法全在 cuav_pfb_cycle.m，本文件只钉死 M。
 *
 * Arguments    : const creal_T w[464]
 *                const double hr[464]
 *                creal_T y[16]
 * Return Type  : void
 */
void cuav_pfb_m16(const creal_T w[464], const double hr[464], creal_T y[16])
{
  creal_T b_hr[464];
  int i;
...
/*
 * File trailer for cuav_pfb_m16.c
 *
 * [EOF]
 */
