...
test('探针 app.chain：不是典型链路时只报 template=null；是的时候给槽位与检查（C-7）', () => {
  ...
  // 十五个槽位都在（D-053 尾部加了测向与多站定位，C-4 加了挂在检测识别评价卡片里的特征提取、
  // 模板识别与评价，C-10 加了挂在接收机前端卡片里的接收滤波）；没有目录时不判断可用性
  assert.equal(Object.keys(a.chain.slots as Record<string, string>).length, 15)
})
