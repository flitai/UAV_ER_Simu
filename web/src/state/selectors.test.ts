...
test('探针 app 子对象含 09 §10 的全部键', () => {
  ...
  const a = probeApp(s, { mapInstanceId: 1, rows: 0, cols: 0, peakBin: null, entities: [], links: [], bearings: [], positions: [] })
  ...
  assert.equal(probeApp(initialState(false, 1920, ''), { mapInstanceId: 1, rows: 0, cols: 0, peakBin: null, entities: [], links: [], bearings: [], positions: [] }).perf, null)
  const withView = probeApp(s, { mapInstanceId: 1, rows: 0, cols: 0, peakBin: null, entities: [], links: [], bearings: [], positions: [], longTasks: { count: 0, maxMs: 0 }, signalView: {
...
test('探针 app.chain：不是典型链路时只报 template=null；是的时候给槽位与检查（C-7）', () => {
  const empty = probeApp(initialState(true, 1920, ''), { mapInstanceId: 1, rows: 0, cols: 0, peakBin: null, entities: [], links: [], bearings: [], positions: [] })
  ...
  // 十一个槽位都在（D-053 尾部加了测向与多站定位）；没有目录时不判断可用性
  assert.equal(Object.keys(a.chain.slots as Record<string, string>).length, 11)
})
