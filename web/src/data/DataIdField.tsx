...
//
// 服务端按机型分组抽样，不给全部 4714 条——挑片段挑的是机型不是某一片。列不全就明说，
// 并给一个筛选框让人找得到具体那一片，不假装列全了（铁律 15）。
    ...
    <div className="data-pick" data-form="data-id">
      ...
      <div className="data-pick-note" data-datasets-note>
        {err
          ? `数据清单取不到：${err}`
          : truncated
            ? `共 ${total} 段，匹配 ${matched} 段，按机型各列前几段；要找具体一段用上面的筛选`
      ...
      {picked?.holdout && (
        <div className="pp-warn" data-datasets-holdout>
          这一段在验收集里。可以回放，但它的结论不能再当作独立验证（D-038）
        </div>
      )}
    </div>
  )
